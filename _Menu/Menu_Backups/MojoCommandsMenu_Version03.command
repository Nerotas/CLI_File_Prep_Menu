...
function HDR_Check_FUNC
{
echo "This command will help techs identify if a file is HDR or SDR. It is currently in testing. Drag in the file:"
echo "-------------------------------------------------"
read -er inputFile
echo "-------------------------------------------------"
ffprobe -v error -pretty -select_streams v:0 -show_streams -i "$inputFile"
echo "-------------------------------------------------"
echo "If the above readout says "color_transfer=smpte2084" the file is HDR within PQ standards."
echo "It requires additional metadata to be SPMTE 2086 and be true HDR."
}
...
echo "-------------------------------------------------"
read -er newSize
convert "$inputFile" -verbose -resize "$newSize"! "${inputFile%.*}_$newSize.jpg"
    ;;
3 ) main_menu ;;
* ) tput setf 2;echo "Please enter 1, 2 or 3";tput setf 2;
esac
...
function main_menu
{
 ...
 UNMARK(){ $e "\e[27m";}
  ...
  ARROW(){ read -s -n3 key 2>/dev/null >&2
     ...
     M1(){ TPUT  5 20; $e "Corruption Detection & Proxy Create";}
     M2(){ TPUT  6 20; $e "HDR Check";}
     M3(){ TPUT  7 20; $e "Hard Link";}
     M4(){ TPUT  8 20; $e "Checksum";}
     M5(){ TPUT  9 20; $e "Tar Package";}
     M6(){ TPUT  10 20; $e "Magick Batch";}
     M7(){ TPUT  11 20; $e "Image Resize";}
     M8(){ TPUT  12 20; $e "Grid Seach";}
     M9(){ TPUT 13 20; $e "Grid Space";}
     M10(){ TPUT 14 20; $e "Proxy Create";}
     M11(){ TPUT 15 20; $e "System Status";}
     M12(){ TPUT 16 20; $e "Fortune Telling";}
     M13(){ TPUT 17 20; $e "ABOUT   ";}
      LM=13
...
REFRESH(){ after=$((i+1)); before=$((i-1))
   ...
   INIT(){ R;HEAD;FOOT;MENU;}
    ...
    while [[ "$O" != " " ]]; do case $i in
        ...
        2) S=M2;SC;if [[ $cur == "" ]];then R;HDR_Check_FUNC;ES;fi;;
        3) S=M3;SC;if [[ $cur == "" ]];then R;HardLink_FUNC;ES;fi;;
        4) S=M4;SC;if [[ $cur == "" ]];then R;Checksum_FUNC;ES;fi;;
        5) S=M5;SC;if [[ $cur == "" ]];then R;Tar_FUNC;ES;fi;;
        6) S=M6;SC;if [[ $cur == "" ]];then R;MagickBatch_FUNC;ES;fi;;
        7) S=M7;SC;if [[ $cur == "" ]];then R;ImageResize_FUNC;ES;fi;;
        8) S=M8;SC;if [[ $cur == "" ]];then R;GridSearch_FUNC;ES;fi;;
        9) S=M9;SC;if [[ $cur == "" ]];then R;GridSpace_FUNC;ES;fi;;
        10) S=M10;SC;if [[ $cur == "" ]];then R;Proxy_FUNC;ES;fi;;
        11) S=M11;SC;if [[ $cur == "" ]];then R;SystemStatus_FUNC;ES;fi;;
        12) S=M12;SC;if [[ $cur == "" ]];then R;FortuneTelling_FUNC;ES;fi;;
        13) S=M13;SC;if [[ $cur == "" ]];then R;$e "\n$($e Template by oTo. Menu built by N. Erotas and is current as of 05/08/2018. DOOM will always be the best game ever.)\n";ES;fi;;
 esac;POS;done
}
